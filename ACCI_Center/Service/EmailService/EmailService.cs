...
namespace ACCI_Center.Service.EmailService
{
   ...
   {       
       ...
       public bool SendEmail(CandidateInformation candidate)
       {
        ...
        public bool SendEmail(ExamRegisterFormViewModel examRegisterFormViewModel)
        {
            // 1. Render Razor HTML
            var engine = new RazorLightEngineBuilder()
                .UseFileSystemProject(Directory.GetCurrentDirectory())
                ...
                .Build();
            string templatePath = "ExamRegisterForm.cshtml";
            string htmlContent = engine.CompileRenderAsync(templatePath, examRegisterFormViewModel).Result;
            ...
            // email.Sender = new MailboxAddress(_mailSettings.DisplayName, _mailSettings.Mail);
            email.From.Add(new MailboxAddress(_mailSettings.DisplayName, _mailSettings.Mail));
            email.To.Add(MailboxAddress.Parse(examRegisterFormViewModel.candidateInformation.Email));
            email.Subject = "Phiếu Dự Thi";
            var builder = new BodyBuilder
            {
                HtmlBody = $"""
                   <p>Xin chào {examRegisterFormViewModel.candidateInformation.HoTen},</p>
                   <p>Trung tâm thông tin đến bạn Phiếu dự thi cho kì thi sắp tới</p>
                   <p>Vui lòng in phiếu dự thi và mang theo cùng giấy tờ cá nhân khi đi thi.</p>
                   <p>Trân trọng,<br/>ACCI_CENTER</p>
               """
